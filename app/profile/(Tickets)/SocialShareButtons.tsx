...
import { SOCIAL_CONFIGS } from "./ticketSocialConfigs";
import { toast } from "sonner";
import { cn } from "@/lib/utils";

// WhatsApp Icon (moved here for encapsulation)
const WhatsAppIcon = () => (
  ...
  <div className={cn("grid grid-cols-4 sm:grid-cols-4 gap-3 mx-auto",className)}>
    {SOCIAL_CONFIGS.map((social: any) => (
      <button
        key={social.platform}
        onClick={() => {
          if (typeof social.onClick === "function") {
            social.onClick(toast);
          } else if (typeof social.getShareUrl === "function") {
            window.open(social.getShareUrl(), "_blank", "noopener,noreferrer");
          }
        }}
        className={
          cn(
