...
export type FrameType = "attendee" | "volunteer" | "speaker"| "organizer";

const YourFrame = ({frameType}:{frameType:FrameType}) => {
  ...
  const drawFrames = useCallback(() => {
    ...
    if (squareCanvas) {
      ...
      if (ctx) {
        ...
        const src=`/images/frames/${frameType}-square.png`
    ...
    if (circleCanvas) {
      ...
      if (ctx) {
        ...
        const banner = new Image();
        banner.src = `/images/frames/attendee-circle.png`;
        banner.crossOrigin = "anonymous";
