...
function getPerformanceTier(points: number) {
  if (points >= 500) return "border-purple-500 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300";
  if (points >= 200) return "border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300";
  if (points >= 100) return "border-green-500 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300";
  if (points >= 50) return "border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300";
  if (points >= 20) return "border-orange-500 bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300";
...
function getInitials(first: string, last: string) {
  return `${first?.[0]?.toUpperCase() || "P"}${last === "." ? "" : last?.[0]?.toUpperCase() || ""}`;
}
...
const PodiumCard = ({ user, rank }: { user: LeaderboardResult; rank: number }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ boxShadow: "0 6px 32px 0 rgba(0,0,0,0.10)", backgroundColor: "rgba(255,255,255,0.98)" }}
      className={cn(
        "relative flex flex-col items-center justify-between bg-gradient-to-br from-card to-card/80 rounded-3xl  p-6 mx-2 min-w-[200px] max-w-[240px] border-2 transition-all duration-300 hover: backdrop-blur-sm",
        rank === 1 && "z-20 scale-110 border-gradient-to-r from-yellow-400 to-yellow-600 bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-yellow-900/30 dark:to-orange-900/30",
        rank === 2 && "z-10 border-gradient-to-r from-gray-400 to-gray-600 bg-gradient-to-br from-gray-50 to-slate-50 dark:from-gray-800/30 dark:to-slate-800/30",
        rank === 3 && "border-gradient-to-r from-orange-400 to-orange-600 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-900/30 dark:to-amber-900/30"
      )}
      style={{ minHeight: 280 }}
    >
      {/* Rank indicator */}
      ...
      <div className="flex flex-col items-center gap-3">
        {medal && (
          <motion.div
            ...
            transition={{ delay: 0.12 * rank, type: "spring", stiffness: 120, damping: 16 }}
            className="relative"
          >
            <Image src={medal} alt={medalLabel || "Medal"} width={56} height={56}  />
      ...
      <div className="flex flex-col items-center gap-2 mt-4">
        <div className={cn("flex items-center gap-2 px-4 py-2 rounded-full font-bold text-lg border-2 ", performanceTier)}>
          <Image src={Coin} alt="points" width={24} height={24} />
      ...
      <div className="flex flex-wrap items-end justify-center w-full gap-4 sm:gap-8 mb-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="animate-pulse">
            <div className="flex flex-col items-center justify-between bg-card rounded-3xl  p-6 mx-2 min-w-[200px] max-w-[240px] border-2" style={{ minHeight: 280 }}>
              <div className="flex flex-col items-center gap-3">
...
const LeaderBoardData = () => {
    ...
    return (
        ...
        <div className="text-center py-12">
          <div className="mb-4">
            ...
            <p className="text-red-500 text-lg font-medium mb-2">Unable to Load Leaderboard</p>
  ...
  const userFirstName = currentUser?.first_name || currentUser?.profile?.first_name || "";
  const userLastName = currentUser?.last_name || currentUser?.profile?.last_name || "";
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center mb-12">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-center sm:items-end justify-center w-full gap-4 sm:gap-8 mb-4 px-2">
          {/* Second Place */}
      ...
      >
        <Table className="rounded-2xl overflow-hidden min-w-[340px] md:min-w-[400px]  text-xs sm:text-sm md:text-base">
          <TableHeader>
            <TableRow className="bg-gradient-to-r from-primary/10 to-primary/5">
              <TableHead className="w-14 sm:w-16 text-center font-bold">Rank</TableHead>
              <TableHead className="font-bold">Participant</TableHead>
              <TableHead className="text-center font-bold">Score</TableHead>
              ...
              return (
                <motion.tr
                  ...
                  className={cn(
                    ...
                    currentRank === 1 && "bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20",
                    currentRank === 2 && "bg-gradient-to-r from-gray-50 to-slate-50 dark:from-gray-800/20 dark:to-slate-800/20",
                    currentRank === 3 && "bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20",
                    isCurrentUser && "border-2 border-blue-500 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30"
                  )}
                >
                    ...
                    {isTopThree ? (
                      <div className="flex items-center justify-center">
                        <Image 
                          ...
                          width={22} height={22} className="inline-block sm:w-7 sm:h-7" 
                        />
                      </div>
                    ) : (
                  ...
                  <TableCell className="text-center p-2 sm:p-3">
                    <div className={cn("inline-flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-full border-2 text-xs sm:text-sm font-bold ", performanceTier)}>
                      <Image src={Coin} alt="points" width={16} height={16} className="sm:w-[18px] sm:h-[18px]" />
