        ...
        <div className="w-full">
              ...
              {hallTabs.map((hall, index) => (
                <div
                  className={cn(
                    ...
                    index === dataIndex &&
                      "border-2 border-l-google-green border-t-google-red border-r-google-yellow border-b-google-blue "
                  )}
                  onClick={() => {
                  ...
                  key={`${hall.name}-${index}`}
                >
                  <span className="flex items-center gap-2">{hall.name}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            {hallTabs.map((hall, hallIndex) => (
              <div
                ref={(el: any) => (dataRefs.current[hallIndex] = el as never)}
                    ...
                    >
                      ...
                      <div className="w-7/10 lg:w-4/5 flex flex-col p-3 border-b border-border grow">
                        <div>
                          <div
                            className={cn(
                              "text-xs px-3 py-1 mb-2 w-fit font-medium flex items-center gap-1 border rounded-full",
                              hallInfo.color,
                              hallInfo.borderColor
                            )}
                          >
                            <span>
                              {isEndingNote ? "Closing" : hallInfo.type}
                            </span>
                          </div>
                          <div
                            className={cn(
                          ...
                          {event.speakers && event.speakers.length > 0 && (
                            <div className="flex items-center gap-2 mt-3 flex-wrap">
                              {event.speakers?.map((speaker: any) => {
                                const speakerData = getSpeakerById(
                                  speakers,
                                  speaker.id
                                );
                                return (
